  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-slate-50 dark:bg-[#1A1D24]">
      ...
      <header className="h-16 flex items-center justify-between px-8 bg-white dark:bg-[#1e2128] border-b border-slate-200 dark:border-slate-700/50 shrink-0 transition-colors duration-200">
        <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Employee Affairs</h1>
      ...
      <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-5">
            {[
              ...
              <div key={label} className="bg-white dark:bg-[#22262f] border border-slate-200 dark:border-slate-700/50 rounded-xl p-5">
        ...
        <div className="flex items-center gap-4 p-4 bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-700/30 rounded-xl">
          <div className="w-9 h-9 rounded-lg bg-purple-100 dark:bg-purple-500/20 flex items-center justify-center shrink-0">
            <DollarSign size={18} className="text-purple-600 dark:text-purple-400" />
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <Link2 size={13} className="text-purple-600 dark:text-purple-400" />
              <span className="text-sm font-bold text-purple-700 dark:text-purple-400">Linked to Accounting</span>
              <span className="px-2 py-0.5 bg-purple-500 text-white text-xs font-bold rounded-full">{payrollExpenseTransactions.length} payroll entries</span>
            </div>
            <p className="text-xs text-purple-600/80 dark:text-purple-400/70 mt-0.5">
              Monthly payroll of <span className="font-bold">${totalMonthlyPayroll.toLocaleString()}</span> across {Object.keys(payrollByDept).length} departments — automatically posted as expenses in the Accounting ledger.
            </p>
          </div>
          <div className="text-right shrink-0">
            <p className="text-xl font-bold text-purple-600 dark:text-purple-400">-${totalMonthlyPayroll.toLocaleString()}</p>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 bg-white dark:bg-[#22262f] border border-slate-200 dark:border-slate-700/50 rounded-xl p-6">
            <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-1">Weekly Attendance Trend</h3>
          ...
          <div className="bg-white dark:bg-[#22262f] border border-slate-200 dark:border-slate-700/50 rounded-xl p-6">
            <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4">Staff by Department</h3>
        ...
        {/* Department Cards */}
        <div>
          <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4">Department Overview</h3>
          <div className="grid grid-cols-3 gap-4">
            {departments.map(dept => (
              <div key={dept.name} className="bg-white dark:bg-[#22262f] border border-slate-200 dark:border-slate-700/50 rounded-xl p-4 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-bold text-slate-800 dark:text-slate-100 text-sm">{dept.name}</p>
        ...
        <div className="bg-white dark:bg-[#22262f] border border-slate-200 dark:border-slate-700/50 rounded-xl p-6">
          <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-1">Department Performance Scores</h3>
          ...
          <div className="p-5 border-b border-slate-100 dark:border-slate-700/50 flex items-center gap-3">
            ...
            <div className="relative ml-auto">
              ...
              <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)}
                className="pl-8 pr-4 py-2 bg-slate-50 dark:bg-[#1A1D24] border border-slate-200 dark:border-slate-600/50 rounded-lg text-sm text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/20" />
            </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-[#1A1D24]">
                {["Employee", "Role", "Department", "Shift", "Status", "Rating", "Salary", ""].map(h => (
            ...
            <tbody>
              {filtered.map(emp => (
                <tr key={emp.id} className="border-b border-slate-50 dark:border-slate-700/20 hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-slate-600 dark:text-slate-400">{emp.role}</td>
                  <td className="px-5 py-3.5 text-slate-500 dark:text-slate-400">{emp.dept}</td>
                  <td className="px-5 py-3.5 text-xs text-slate-500 dark:text-slate-400 font-mono">{emp.shift}</td>
                  ...
                  <td className="px-5 py-3.5 font-semibold text-slate-700 dark:text-slate-300">${emp.salary.toLocaleString()}</td>
                  <td className="px-5 py-3.5">
